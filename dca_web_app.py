...
start   = st.number_input("Start Day", 0, value=0)
ignore  = st.text_input("Ignore Days (e.g. 200-220, 300)")
...
if file:
    ...
    if go_btn:
        ig=parse_ignore(ignore)
        hist=df[df['Days']>=start].copy()
        if ig: hist=hist[~hist['Days'].isin(ig)]

        t_yrs=(hist['Days']-hist['Days'].iloc[0]) / 365.25
        q    =hist['Qo'].values
        mask =(q>0) & ~np.isnan(q)
        t,q  =t_yrs[mask], q[mask]
        ...
        # forecast 100 yr horizon
        horizon_days=np.arange(0,int(100*365.25))
        yrs=horizon_days/365.25
        qo=f(yrs); cum=np.cumsum(qo)
        EUR=eur_mcm*1e6

        stop=((qo<cutoff)|(cum>EUR))&(yrs>t[-1])
        end=np.argmax(stop) if stop.any() else len(horizon_days)

        fd=horizon_days[:end]; fq=qo[:end]; cum=cum[:end]; x0=hist['Days'].iloc[0]
